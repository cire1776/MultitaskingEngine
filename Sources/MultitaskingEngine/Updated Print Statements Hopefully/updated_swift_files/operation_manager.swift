...
//                logger.log(level: LogLevel.info, message: "⏳ Pump retry called, but queue is empty. Doing nothing.")
...
//            }
    ...
    func addOperation(_ operation: any OperationExecutable) async -> Bool {
        ...
        if nextTail == head {
            logger.log(level: LogLevel.info, message: "⚠️ Queue full! Cannot add operation: \(operation.operationName)")
            return false
        }

        logger.log(level: LogLevel.info, message: "📌 Adding operation: \(operation.operationName) (Queue size: \(calculateQueueSize()))")
        ...
        if let continuation = waitingContinuation {
            logger.log(level: LogLevel.info, message: "🔄 Waking up OM!")
            waitingContinuation = nil
            continuation.resume()  // ✅ Wake up OM immediately
        } else {
            logger.log(level: LogLevel.info, message: "⚠️ OM was not waiting. This might be a bug.")
        }
    ...
    func start() async {
        ...
        repeat {
            if head == tail && !isPumping {
                logger.log(level: LogLevel.info, message: "⏳ OM is waiting for new tasks...")
                await withCheckedContinuation { continuation in
                    waitingContinuation = continuation
                }
                logger.log(level: LogLevel.info, message: "✅ OM resumed!")
            }
        ...
        isRunning = false
        isPumping = false
        logger.log(level: LogLevel.info, message: "✅ OperationManager has stopped.")
    }
    ...
    private func processNextOperation() async {
        ...
        if head == tail {
            ...
            if !nextCycleQueue.isEmpty {
                logger.log(level: LogLevel.info, message: "📥 Moving operations from nextCycleQueue to mainQueue...")
                ...
                logger.log(level: LogLevel.info, message: "✅ Next cycle operations transferred. Continuing execution.")
            } else {
                logger.log(level: LogLevel.info, message: "⏳ No operations left. Suspending OM...")
        ...
        logger.log(level: LogLevel.info, message: "🚀 Executing operation: \(operation.operationName) | State: \(operation.state)")
    ...
    private func processResult(_ operation: OperationExecutable, _ result: OperationState) async {
        ...
        switch result {
            case .firstRun:
                logger.log(level: LogLevel.info, message: "🔄 OM Processing firstRun -> running")
            ...
            case .exception(let message):
                ...
                if shouldResume {
                    logger.log(level: LogLevel.info, message: "🔄 Resuming operation: \(operation.operationName)")
                    operation.state = .running
                    nextCycleQueue.append(operation)  // ✅ Now safely handled in addOperation()
                }

            case .completed, .abort:
                logger.log(level: LogLevel.info, message: "🏁 Operation \(operation.operationName) finished execution. No further action.")
    ...
    func stopNow() async {
        ...
        logger.log(level: LogLevel.info, message: "🚨 OperationManager stopped immediately.")
    }
